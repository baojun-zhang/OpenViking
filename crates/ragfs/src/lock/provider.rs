...
mod tests {
    ...
    #[tokio::test]
    async fn memory_provider_create_and_read() {
        let p = MemoryPathLockProvider::new();
        let token = LockToken {
            owner_id: "owner1".to_string(),
            time_ns: 100,
            lock_type: super::super::types::PathLockKind::Exact,
        };
        p.try_create_token("/lock", &token).await.unwrap();
        let read = p.read_token("/lock").await.unwrap().unwrap();
        assert_eq!(read.owner_id, "owner1");
    }

    #[tokio::test]
    async fn memory_provider_conflict() {
        let p = MemoryPathLockProvider::new();
        let token = LockToken {
            owner_id: "owner1".to_string(),
            time_ns: 100,
        ...
        p.try_create_token("/lock", &token).await.unwrap();
        let token2 = LockToken {
            owner_id: "owner2".to_string(),
            time_ns: 200,
            lock_type: super::super::types::PathLockKind::Exact,
        };
        assert!(p.try_create_token("/lock", &token2).await.is_err());
    }

    #[tokio::test]
    async fn memory_provider_refresh_and_remove() {
        let p = MemoryPathLockProvider::new();
        let token = LockToken {
            owner_id: "owner1".to_string(),
            time_ns: 100,
            lock_type: super::super::types::PathLockKind::Tree,
        };
        p.try_create_token("/lock", &token).await.unwrap();
        assert!(p.refresh_token("/lock", "owner1", 200).await.unwrap());
        assert!(!p.refresh_token("/lock", "wrong", 300).await.unwrap());
        assert!(p.remove_token("/lock", "owner1").await.unwrap());
        assert!(p.read_token("/lock").await.unwrap().is_none());
    }

    #[tokio::test]
    /// Verify filesystem provider delegates refresh/remove CAS to LocalFS compare operations.
    async fn filesystem_provider_uses_localfs_compare_and_write_remove() {
        let dir = tempfile::TempDir::new().unwrap();
        let fs = crate::plugins::localfs::LocalFileSystem::new(dir.path().to_str().unwrap()).unwrap();
        let p = FilesystemPathLockProvider::new(Arc::new(fs));
        let token = LockToken {
            owner_id: "owner1".to_string(),
            time_ns: 100,
            lock_type: super::super::types::PathLockKind::Exact,
        ...
        p.try_create_token("/lock", &token).await.unwrap();
        assert!(!p.refresh_token("/lock", "wrong", 200).await.unwrap());
        assert!(p.refresh_token("/lock", "owner1", 200).await.unwrap());

        let read = p.read_token("/lock").await.unwrap().unwrap();
        assert_eq!(read.owner_id, "owner1");
        assert_eq!(read.time_ns, 200);

        assert!(!p.remove_token("/lock", "wrong").await.unwrap());
        assert!(p.remove_token("/lock", "owner1").await.unwrap());
        assert!(p.read_token("/lock").await.unwrap().is_none());
    }

    #[tokio::test]
    async fn mounted_filesystem_descendant_lock_blocks_ancestor_tree() {
        let fs = Arc::new(MountableFS::new());
        fs.register_plugin(MemFSPlugin).await;
        fs.mount(PluginConfig::single_backend(
            "memfs",
            "/mem",
            HashMap::new(),
        ))
        .await
        .unwrap();
        fs.mkdir("/mem/root", 0o755).await.unwrap();
        fs.mkdir("/mem/root/child", 0o755).await.unwrap();

        let provider = Arc::new(FilesystemPathLockProvider::new(fs.clone()));
        let manager = PathLockManager::new(fs, provider, PathLockConfig::default());
        let child = manager
            .acquire_tree("/mem/root/child", Duration::ZERO, Some("child-owner"))
            .await
            .unwrap();

        let ancestor = manager
            .acquire_tree("/mem/root", Duration::ZERO, Some("ancestor-owner"))
            .await;
        assert!(
            matches!(ancestor, Err(PathLockError::Timeout { .. })),
            "ancestor tree lock must observe the mounted descendant lock"
        );
        manager.release(&child).await.unwrap();
    }
}
