...
| 参数 | 类型 | 说明 | 默认值 |
|------|------|------|--------|
| `workspace` | str | 本地数据存储路径（主要配置），会覆盖 `agfs.path` 和 `vectordb.path` | "./data" |
| `agfs` | object | agfs 配置 | {} |
...
| 参数 | 类型 | 说明 | 默认值 |
|------|------|------|--------|
| `path` | str | [已废弃，使用 workspace] AGFS 数据存储路径 | null |
| `port` | int | AGFS 服务端口 | 1833 |
| `log_level` | str | AGFS 日志级别 | "warn" |
| `url` | str | 服务模式下的 AGFS 服务 URL | "http://localhost:1833" |
| `backend` | str | AGFS 存储后端: 'local'  's3'  'memory' | "local" |
| `timeout` | int | AGFS 请求超时（秒） | 10 |
| `retry_times` | int | AGFS 失败时的重试次数 | 3 |
| `use_ssl` | bool | 为 AGFS 服务启用/禁用 SSL（HTTPS） | true |
| `s3` | object | S3 后端配置（当 backend 为 's3' 时使用） | - |
...
| 参数 | 类型 | 说明 | 默认值 |
|------|------|------|--------|
| `backend` | str | VectorDB 后端类型: 'local'（基于文件）, 'http'（远程服务）, 'volcengine'（VikingDB）或 'vikingdb'（私有部署） | "local" |
| `name` | str | VectorDB 的集合名称 | "context" |
| `path` | str | [已废弃，使用 workspace] 'local' 类型的本地存储路径 | null |
...
**vectordb.volcengine 配置**

| 参数 | 类型 | 说明 | 默认值 |
|------|------|------|--------|
| `ak` | str | 火山引擎访问密钥 | null |
| `sk` | str | 火山引擎秘密密钥 | null |
| `region` | str | 火山引擎区域（例如 'cn-beijing'） | null |
| `host` | str | 火山引擎 VikingDB 主机（可选） | null |

**vectordb.vikingdb 配置**

| 参数 | 类型 | 说明 | 默认值 |
|------|------|------|--------|
| `host` | str | VikingDB 服务主机 | null |
| `headers` | dict | 请求的自定义头 | {} |
