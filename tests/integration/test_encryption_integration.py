...
"""
    ...
    async def _check_all_files_encrypted(self, root_uri, ctx, svc, test_data_dir: Path, prefix="  "):
        """
...
"""
        ...
        try:
            ...
            raw_content = self._backend_file_bytes(svc, ctx, data["test_data_dir"], relation_file_uri)
            assert raw_content.startswith(b"OVE1"), (
                ...
                for entry in entries:
                    ...
                    if entry["isDir"]:
                        ...
                        if ".relations.json" in entry_name:
                            continue
                        try:
                            raw_content = self._backend_file_bytes(svc, ctx, data["test_data_dir"], entry_uri)
                            assert raw_content.startswith(b"OVE1"), (
