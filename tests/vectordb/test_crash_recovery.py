...
def worker_write_and_crash(path, start_id, count, event_ready):
    ...
    try:
        ...
        print(f"[Subprocess] Upsert done. Not closing.")
...
class TestCrashRecovery(unittest.TestCase):
    ...
    def test_simple_crash_recovery(self):
        ...
        self.assertEqual(len(res.items), data_count, f"Should find all {data_count} items in KV Store")
        ...
        search_res = col.search_by_vector(
            "idx_crash", 
            dense_vector=[0.1]*4, 
            limit=data_count
        )
        ...
        self.assertEqual(len(found_ids), data_count, "Index should contain all items after recovery")
    ...
    def test_multi_cycle_crash(self):
        ...
        found_ids = set(item.id for item in res_search_old.data)
        for i in range(100):
        ...
        found_ids_new = set(item.id for item in res_search_new.data)
        for i in range(1000, 1300):
            self.assertIn(i, found_ids_new, f"ID {i} should exist")
        
        print(f"[Main] Total items verified via index: 700")
