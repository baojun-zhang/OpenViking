...
class StressStats:
    ...
    def report(self):
        with self.lock:
            ...
            print(f"Insert: {self.insert_count} ops, {self.insert_count/duration:.2f} OPS, Avg Latency: {np.mean(self.insert_latency) if self.insert_latency else 0:.4f}s")
            print(f"Search: {self.search_count} ops, {self.search_count/duration:.2f} OPS, Avg Latency: {np.mean(self.search_latency) if self.search_latency else 0:.4f}s")
            print(f"Delete: {self.delete_count} ops, {self.delete_count/duration:.2f} OPS, Avg Latency: {np.mean(self.delete_latency) if self.delete_latency else 0:.4f}s")
...
def setup_collection(path: str, dim: int, enable_sparse: bool):
    ...
    col.create_index("idx_vector", {
        "IndexName": "idx_vector",
        "VectorIndex": vector_index_config
    })
...
def worker_insert(col: Collection, stats: StressStats, start_id: int, count: int, batch_size: int, dim: int, enable_sparse: bool, stop_event: threading.Event = None):
    current_id = start_id
...
def worker_search(col: Collection, stats: StressStats, duration: int, dim: int, max_id: int, enable_sparse: bool, stop_event: threading.Event = None):
    ...
    while time.time() < end_time:
        ...
        try:
            ...
            if enable_sparse:
                ...
                col.search_by_vector("idx_vector", generate_random_vector(dim), limit=10, sparse_vector=sparse_q)
            else:
            ...
            if enable_sparse:
                sparse_q = generate_random_sparse_vector()
                col.search_by_vector("idx_vector", generate_random_vector(dim), limit=10, filters=filter_cond, sparse_vector=sparse_q)
            else:
                col.search_by_vector("idx_vector", generate_random_vector(dim), limit=10, filters=filter_cond)
...
def worker_delete(col: Collection, stats: StressStats, duration: int, max_id: int, stop_event: threading.Event = None):
    end_time = time.time() + duration
    while time.time() < end_time:
        if stop_event and stop_event.is_set():
            break
...
def run_stress_test():
    ...
    print(f"=== Starting Stress Test (Dim={args.dim}, Duration={args.duration}s, Sparse={args.enable_sparse}) ===")
    ...
    worker_insert(col, stats, 0, args.initial_data, batch_size=100, dim=args.dim, enable_sparse=args.enable_sparse)
    ...
    for i in range(args.insert_threads):
        ...
        t = threading.Thread(target=worker_insert, args=(col, stats, t_start, items_per_thread, 50, args.dim, args.enable_sparse, stop_event))
        t.daemon = True
        t.start()
        threads.append(t)
    ...
    for i in range(args.search_threads):
        t = threading.Thread(target=worker_search, args=(col, stats, args.duration, args.dim, estimated_max_id, args.enable_sparse, stop_event))
        t.daemon = True
        t.start()
        threads.append(t)
        
    # Start Delete Threads
    for i in range(args.delete_threads):
        t = threading.Thread(target=worker_delete, args=(col, stats, args.duration, estimated_max_id, stop_event))
        t.daemon = True
        t.start()
        threads.append(t)
