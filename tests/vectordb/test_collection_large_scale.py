...
class TestLargeScaleScenarios(unittest.TestCase):
    ...
    def test_01_large_batch_insert_10k(self):
        ...
        for batch_idx in range(total_records // batch_size):
            ...
            print(f"  Batch {batch_idx + 1}/{total_records // batch_size} inserted ({len(result.ids)} records)")
        ...
        print(f"✓ Filtered search completed in {filter_time*1000:.2f}ms (returned {len(result.data)} results)")
    ...
    def test_05_massive_deletes(self):
        ...
        self.assertEqual(set([item.id for item in fetch_result.items]), {25001, 50000})
        ...
        self.assertLess(actual_count, 30000, "Search should return less than 30000 records after deletion")
        ...
        for deleted_id in deleted_samples:
            self.assertNotIn(deleted_id, search_ids, f"Deleted ID {deleted_id} should not be in search results")
        ...
        self.assertEqual(agg_count, 25000, f"Expected 25000 records after deletion, but aggregate_data returned {agg_count}")
    ...
    def test_06_complex_multi_filter_large_scale(self):
        ...
        query_vec = [random.gauss(0, 1) for _ in range(128)]
        search_start = time.time()
        result1 = collection.search_by_vector("idx_complex", dense_vector=query_vec, limit=100, filters=filter1)
        search_time1 = time.time() - search_start
        print(f"  Query 1 (4 conditions): {len(result1.data)} results in {search_time1*1000:.2f}ms")
        ...
        search_start = time.time()
        result2 = collection.search_by_vector("idx_complex", dense_vector=query_vec, limit=100, filters=filter2)
        search_time2 = time.time() - search_start
        print(f"  Query 2 (3 conditions): {len(result2.data)} results in {search_time2*1000:.2f}ms")
        ...
        print(f"  Query 3 (scalar sort): {len(result3.data)} results in {search_time3*1000:.2f}ms")
    ...
    def test_07_large_scale_aggregation(self):
        ...
        print(f"  Country aggregation (50 groups): {len(result1.agg)} groups in {agg_time1*1000:.2f}ms")
        ...
        print(f"  Product aggregation (100 groups): {len(result2.agg)} groups in {agg_time2*1000:.2f}ms")
        ...
        print(f"  City aggregation (200 groups): {len(result3.agg)} groups in {agg_time3*1000:.2f}ms")
        ...
        result4 = collection.aggregate_data(
            ...
            op="count",
            field="product",
            filters={"op": "must", "field": "country", "conds": ["country_0", "country_1", "country_2"]}
        )
    ...
    def test_08_persistence_with_large_data(self):
        ...
        result_before = collection1.search_by_vector("idx_persist", dense_vector=query_vec, limit=50)
        ...
        collection2 = self.register_collection(
            get_or_create_local_collection(path=TEST_DB_PATH)
        )
    ...
    def test_09_concurrent_operations_simulation(self):
        ...
        for op_type, op_data in operations:
            if op_type == "search":
                result = collection.search_by_vector("idx_concurrent", dense_vector=op_data, limit=10)
                search_count += 1
            elif op_type == "insert":
    ...
    def test_10_stress_test_continuous_operations(self):
        ...
        collection.create_index("idx_stress", {"IndexName": "idx_stress", "VectorIndex": {"IndexType": "flat"}})
        ...
        while time.time() - start_time < test_duration:
            ...
            op = random.choice(["search", "search", "search", "insert", "update"])  # Higher weight for search
        ...
        if search_times:
            ...
            print(f"  Search latency: avg={avg_search*1000:.2f}ms, min={min_search*1000:.2f}ms, max={max_search*1000:.2f}ms")
